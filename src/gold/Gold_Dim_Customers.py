...
CONFIG = {
    "silver_scd2_table" : f"{env.catalog}.{env.dataset}_silver.conform_customers",

    "gold_dim_table" : f"{env.catalog}.{env.dataset}_gold.dim_customers_hist",
    "gold_dim_path" : f"{env.curated_base_path}/{env.dataset}/customers/dim_customers_hist"
}
...
PIPELINE_NAME = "gold_dim_customers_hist"
DATASET = "dim_customers_hist"
...
try:
    ...
    incr_silver_df = silver_df.filter(col("updated_at") > lit(last_wm))

    if incr_silver_df.limit(1).count() == 0:
        spark.sql(f"""
    ...
    new_wm = incr_silver_df.agg(max(col("updated_at")).alias("mx")).collect()[0]["mx"]

    gold_batch = (incr_silver_df
        .select(
            col("customer_sk").cast("bigint").alias("customer_sk"),
            "customer_id",
            "email",
            "city",
            "state",
    ...
    spark.sql(f"""
                ...
                SET finished_at = current_timestamp(),
                    ...
                    rows_in = {rows_in},
                    rows_out = {rows_out},
                    last_watermark_ts = {new_wm.isoformat(sep=" ")}
                WHERE pipeline_name = '{PIPELINE_NAME}' AND dataset = '{DATASET}' AND run_id = '{RUN_ID}'
            """)
